...
class SubtitleApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Voice Chatbot with Subtitles")

        self.subtitles_label = tk.Label(master, text="", font=("Helvetica", 16))
        self.subtitles_label.pack(pady=20)
...
def update_subtitles(text):
    subtitle_app.subtitles_label.config(text=text)
            ...
            if wakeword_queue.get():
                ...
                while True:
                    ...
                    if time.time() - last_query_timestamp > 180:
                        print("No queries for 3 minutes. Returning to the main loop.")
                        stop_gui_process1()
                        start_gui_process()
                        return main_loop()
            else:
